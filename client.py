...
class FixClient(fix.Application):
    ...
    def toAdmin(self, message, sessionID):
        ...
        if msg_type.getValue() == fix.MsgType_Logon:
            print(f"Sending Logon message to {sessionID} {message}")
        elif msg_type.getValue() == fix.MsgType_Logout:
            print(f"Sending Logout message to {sessionID} {message}")
    ...
    def fromAdmin(self, message, sessionID):
        ...
        if msg_type.getValue() == fix.MsgType_Logon:
            print(f"Received Logon response from {sessionID} {message}")
        elif msg_type.getValue() == fix.MsgType_Logout:
            print(f"Received Logout response from {sessionID} {message}")
        elif msg_type.getValue() == fix.MsgType_Heartbeat:
            print(f"Received Heartbeat from {sessionID} {message}")
            ...
            print(f"Received Resend Request{message}")
        elif msg_type.getValue() == "4":
            # Handle Sequence Reset (35=4)
            print(f"Received Sequence Reset Request {message}")
        elif msg_type.getValue() == "1":
            # Handle Test (35=1)
            print(f"Received Test Request {message}")
        else:
    ...
    def generate_clordid(self):
        ...
        # Format the timestamp: YYYYMMDD-HHMMSS-SSS
        clordid_timestamp = now.strftime("%Y%m%d-%H%M%S-%f")[:21]  # Truncate microseconds to milliseconds

        # Combine with the sequence number (optional)
        clordid = f"order-{clordid_timestamp}-{self.order_count+1:03d}"
    ...
    def send_order(self, symbol, side, price=None):
        ...
        order.setField(fix.ClOrdID(ClOrdID))  # Set client order ID
    ...
    def cancel_order(self, cl_ord_id):
        ...
        cancel_request.setField(fix.ClOrdID(self.generate_clordid()))  # Unique ID for the cancel request
    ...
    def handle_reject(self, message):
        # print(message)
        refSeqNumField = fix.RefSeqNum()
        message.getField(refSeqNumField)  # This will populate the `refSeqNumField` object
        refSeqNo = refSeqNumField.getValue()  # Retrieve the actual value from the field object
        
        reasonField = fix.Text()
        message.getField(reasonField)
        reason = reasonField.getValue()
        
        refTagIDField = fix.RefTagID()
        message.getField(refTagIDField)
        refTagID = refTagIDField.getValue()
        
        print(f"Order rejected. RefSeqNum: {refSeqNo}, Reason: {reason}, RefTagId:{refTagID}")
